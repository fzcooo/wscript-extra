...
export function query(wql) {
  let rs = exec(wql);
  let rows = map(new Enumerator(rs), record => {
    return reduce(
      new Enumerator(record.Properties_),
      (rowData, prop) => {
        if (prop.Value != null) {
...
export function eachRow(wql, cb) {
  let rs = exec(wql);
  each(new Enumerator(rs), it => cb(it));
  rs = null;
}
...
export function ps(l) {
  ...
  return map(new Enumerator(rs), it => {

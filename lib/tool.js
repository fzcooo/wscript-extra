...
export function each(x, fn) {
  let i, key, len;
  if (isArrayLike(x)) {
    for (i = 0, len = x.length; i < len; i++) {
      if (fn(x[i], i, x) === false) break;
    }
  } else if (x instanceof Enumerator) {
    for (i = 0; !x.atEnd(); x.moveNext(), i++) {
      if (fn(x.item(), i, x) === false) break;
    }
  } else if (isLength(x)) {
    for (i = 0; i < x; i++) {
      if (fn(i, i, x) === false) break;
    }
  } else if (isObject(x)) {
    for (key in x) {
      if (has(x, key)) {
        if (fn(x[key], key, x) === false) break;
      }
    }
  }
  return x;

...
export function mapKeys(obj, fn = identity) {
  return reduce(
    obj,
    (result, v, k) => {
      result[fn(k)] = v;
...
export function partial(fn, ...boundArgs) {
  return function(..._args) {
    let i = 0;
    let args = boundArgs.map(arg => (arg === PLACE_HOLDER ? _args[i++] : arg));
...
export function curry(fn, arity = fn.length) {
  const create = function(storedArgs) {
    return function(...args) {
      let i = 0;
      let _args = storedArgs.map(it => (it === PLACE_HOLDER ? args[i++] : it));
      if (i < args.length) {
        _args = _args.concat(args.slice(i));
      }
      if (!_args.includes(PLACE_HOLDER) && _args.length >= arity) {
        fn(..._args);
      } else {
        return create(_args);
      }
...
export function pipe(target) {
  const next = function(storedArgsList) {
    return function(...args) {
      if (args.length > 0) {
        storedArgsList.push(args);
        return next(storedArgsList);
      } else {
        return storedArgsList

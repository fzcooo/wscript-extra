...
class Parser {
  ...
  _result() {
    ...
    if (this.cached < this.cursor - 1) {
      this._row();
    }
    len = this.rows.length;
    result = this.rows;
    if (this.header === true && len > 1) {
      header = this.rows.shift();
    } else if (isArray(this.header) && len > 0) {
      header = this.header;
    }
    if (header && header.length > 0) {
      result = this.rows.map((row) => zipObject(header, row));
    }
    return result;
  }
